...
export const useWooProducts = (
    ...
    useEffect(() => {
        ...
        const fetchAllProducts = async () => {
            ...
            if (
                catCache &&
                catCache.products &&
                now - catCache.lastFetch < catCache.cacheDuration
            ) {
                console.log(
                    `Используем кэшированные товары для категории ${categoryId}`,
                );
            ...
            if (!CONSUMER_KEY || !CONSUMER_SECRET) {
                console.warn(
                    "Ключи API не настроены — использую пустые данные",
                );
                setAllProducts([]);
                setLoading(false);
                return;
    ...
    const uniqueOptions = useMemo(() => {
        ...
        allProducts.forEach((product) => {
            if (product.gender && product.gender !== "Унисекс")
                genders.add(product.gender);
            if (product.country && product.country !== "Не указано")
                countries.add(product.country);
            if (product.type && product.type !== "Не указан")
                frameTypes.add(product.type);
            if (product.brand && product.brand !== "Без бренда")
        ...
        return {
            ...
            materialOptions: Array.from(materials).map((m) => ({
                value: m,
                label: m,
            })),
            frameTypeOptions: Array.from(frameTypes).map((t) => ({
                value: t,
                label: t,
            })),
            colorOptions: fixedColorOptions,
            brandOptions: Array.from(brands).map((b) => ({
                value: b,
                label: b,
            })),
        };
    ...
    const filteredProducts = useMemo(() => {
        ...
        const filtered = allProducts.filter((product) => {
            ...
            if (
                ...
                !filters.material.some((mat) => product.material.includes(mat))
            )
                return false;
            if (
